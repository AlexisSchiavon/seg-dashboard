...
**Domain:** Python backend (FastAPI) for a talent-agency commercial intelligence dashboard — JWT auth, SQLite/SQLAlchemy, third-party API integrations (Pipedrive, Google Sheets, Trello), Anthropic Claude for PDF reports + NL agent, Docker/EasyPanel deploy.
**Researched:** 2026-06-09
**Confidence:** HIGH (core framework/auth/DB), MEDIUM (Anthropic SDK structured-output specifics), MEDIUM-LOW (Pipedrive/Trello clients — no good official Python SDKs, recommendation is to avoid third-party wrappers)
...
```bash
# Core (M1)
pip install "fastapi[standard]" "uvicorn[standard]" sqlalchemy alembic \
...
# AI / Reports (M5-M6)
pip install anthropic weasyprint jinja2

# Dev dependencies
pip install -D pytest ruff
```

Note: `fastapi[standard]` pulls in `uvicorn[standard]`, `jinja2`, `python-multipart`, and `httpx` already — listing them explicitly above is for clarity on what's actually used and why.
...
**For M1 (FastAPI base + JWT + health check):**
...
**For M2-M4 (Pipedrive, Sheets, Trello integrations):**
...
**For M5 (PDF reports with Claude):**
...
**For M6 (NL agent over internal data):**
...
---
*Stack research for: SEG Talent Intelligence Dashboard — Python/FastAPI backend*
*Researched: 2026-06-09*
